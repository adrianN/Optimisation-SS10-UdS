...
Since $B\delta = 0$ so the active constraints are still active. For the inactive constraints we have some slack before we leave the polyhedron. If we choose $\epsilon$ small enough we're still within. It suffices to choose $\epsilon$ such that 

\[\forall i: \epsilon |c_i z| < c_i x - f_i\]
...
A LP in standard form is of the form:
\begin{eqnarray*}
\text{minimize:}& \vec c \cdot \vec x \\
...
Any LP can be transformed into an equivalent LP in standard form. Two steps are needed for this, one to add non negativity constraints for all variables and a second to transform any inequalities into an equality.
