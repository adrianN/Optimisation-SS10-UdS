...
\begin{equation}\label{Eq:threeKindsConstraints}P=\left\{x\left| \begin{array}{cr}
...
\begin{align*}
\forall i\in T_1: a_{in}&>0\\
\forall i\in T_2: a_{in}&<0\\
\forall i\in T_3: a_{in}&=0
\end{align*}

By just pulling out the last summand $a_{in}x_n$ out of the calculation.

Let $d_i$ be scalars and $f_i$ be vectors. The solving the equations in \ref{Eq:threeKindsConstraints} for $x_n$, the polyhedron can be rewritten to use the following constraints:
...
Now we can get rid of $x_n$ by putting equations \ref{For:Motz1} and \ref{For:Motz2} together. Hence the projection $\overline P=Q$ can be written as:\footnote{Adrians lecture notes as well as mine read: $\{d_i+f_i\bar x \leq d_j+f_j\bar x\ \forall i\in T_2, j\in T_1 $. The book says $\geq$ and $T_2\geq x_n \geq T_1$. Makes sense if you look at equations \ref{For:Motz1} and \ref{For:Motz2}}

\begin{align*}
Q=\{\bar x | d_i+f_i\bar x &\geq d_j+f_j\bar x     &&\forall i\in T_2, j\in T_1 \\ 
...
\begin{thm}\label{Thm:linTransPoly} Let $P\in \R^n$ be a polyhedron and $A\in \R^{m\times n}$ be a matrix. Then 
\[\{Ax|x\in P\}\]
is a polyhedron.
\end{thm}
...
\[Q=\{(Ax,x)\in R^{m+n}| x\in P\}\]  %this is actually not that intuitive for me. Why is this a polyhedron?
