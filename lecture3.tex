...
To prove that this version implies the one from theorem \ref{Thm:farkasI} let $A'= [A|-A|-I], x'=[x_1|x_2|z]$ such that $x_1,x_2,z\geq 0$ we have

\begin{align*}
A'x' =b &\Rightarrow A(x_1-x_2)-z=b\\
	&\Rightarrow A(x_1-x_2) = b+z\\
	&\Rightarrow Ax^* \geq b
\end{align*}

Which proves the first condition. For the other condition we want to prove that not the second condition is \ref{Thm:farkasII} implies not the second condition in \ref{Thm:farkasI} 

\[A'x' = b \Rightarrow \neq (\exists y, y'A'\geq 0 \wedge \trans yb <0)\]


\begin{align*}
\trans y A' \geq 0 \wedge \trans y b<0\\
[\trans y A | -\trans y A | -\trans y] \geq 0\wedge \trans y b<0\\
\underbrace{\trans y A \geq 0 \wedge -\trans y A \geq 0}_{\Rightarrow \trans y A = 0} \wedge \trans y \geq 0\wedge \trans y b < 0\\
\end{align*}

Now we can choose $y*=-y$ and get $\trans y* b > 0$ and $\trans y*A=0$ which is the proof for

\[A'b'=b,x\geq 0 \Rightarrow \neq (\exists y, y\geq 0 \trans yA=0, \trans yb >0\]
