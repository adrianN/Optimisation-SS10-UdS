...
\begin{eqnarray*}
 & (x_0,\ldots,x_n) & \\
s.t. & A\trans{(x_0,\ldots,x_n)} & \geq b \\
& c\trans{(x_0,\ldots,x_n)} & = x_0
\end{eqnarray*}
...
0 \geq \trans y b
\end{align*}
Since $\trans y b<0$ its a contradiction.
...
\begin{thm}[Farkas' lemma II] Let A be a $m\timen n$ matrix and $b$ a vector. Then exactly one of the following is true:
\begin{enumerate}
...
\[C(A)=\hx|x=\sum^m \lambda_i a_i;\ \lambda_i \geq 0\}\]
\end{Def}
