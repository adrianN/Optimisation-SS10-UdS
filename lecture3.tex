...
\end{eqnarray*}
Those constrains are shown below the x-axis with broken lines. Note that we're only interested in the red part. So the two constraint $c_2 \geq c_3$ and $c_1 \geq c_4$ would be enough, but as there is no efficent way to find those two constraints. We have to keep all of them, which is a problem for higher dimensions.
