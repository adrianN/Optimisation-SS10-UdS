...
\begin{thm}[Farkas' lemma II] Let A be a $m\times n$ matrix and $b$ a vector. Then exactly one of the following is true:
\begin{enumerate}
...
or there is a direction $y$ in $A$ that splits the space between the cone of $A$ and $b$. Note that $\trans y A\leq 0$ means that all columns of $A$ have a negative dot product with $y$. So all positive linear combinations of the columns (i.e. all points in the cone) are on the opposite side from $b$ of the hyperplane defined by $y$.