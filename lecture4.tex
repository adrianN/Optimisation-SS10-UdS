...
\begin{pr}[Theorem \ref{Pr:simplexIIIopt}] Let $x$ be the basiv feasible solution induced by $B$, $y$ be any other feasible solution. We want to argue that the cost of $x$ is smaller than the cost of $y$. %Let $d=y-x$ then 
...
\[cd=c_{B}^{T} d_b + \sum_{k \not \in B} {d_k c_k}= \sum_{k \not \in B} {d_k (c_k-A_{B}^{-1} A_k)}=\sum_{k \in B} {d_k \bar c_k}\]
