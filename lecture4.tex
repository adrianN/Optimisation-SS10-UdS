...
\begin{center}
\begin{align*}
x_{b_i} +\Theta d_{b_i} & \stackrel{!}{\geq} 0 &&\forall i %is ! because we want to choose theta s.t. it holds
...
However we'd like to move as far as possible along the line $y=x+\Theta d$ without leaving the polyhedron to we end up at a vertex. So we choose it like this
...
Notice that for all $k \in B$ we have $x_k=0$ and $y_k \geq 0$ and therefore $d_k \geq 0$. By our assumption, all reduced costs are non-negative. Thus, $c y - c x = c d \geq 0$. Since this holds for all feasible $y$, it follows that $x$ is indeed optimal.
\end{pr}
