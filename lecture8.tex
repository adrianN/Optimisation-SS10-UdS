...
\[\max_x\min_y\E_{{i~x}\atop {j~y} [D_{ij}] \leq \min_\max_x\E_{{i~x}\atop {j~y} [D_{ij}]\]
...
\begin{align*}
\E_{{i~x}\atop {j~y} [D_{ij}] &= \sum_{i,j} x_i D_{ij}y_j\\
...
\[\max_{i\in I} \E_{a~y}[D(i,a)] = \max_{i\in I} \sum_{a\in A} y_a D(i,a)\]
\end{Def}
...
\[\min_y \max_i \E_{a~y}[D(i,a)] \leq \max_x\min_{a\in A} \E_{i~x}[D(i,a)]\]
...
Fix an algorithm $a$ and look at $\E_{i~x}[D(i,u)]$. We can focus on what happens in a single column $b$, since the columns are indepedent from each other. The algorithm has to read entries in the column until it reads an one. If it checks an entry it is a one with probability $\frac{1}{n}$. The probability that it does $k$ steps is

\[\frac{1}{n} + \frac{n-1}{n}\left( \frac{1}{n-1}^2+\frac{n-2}{n-1}\left(\ldots = \frac{1}{n} + \frac 2n + \frac 3n + \ldots= \frac{n+1}{2}\]
