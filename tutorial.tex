...
Prominent examples for linear transformations are rotations and reflections around the origin. If we use a standard basis for our vector space we can easily represent linear transformations by their effects on the basis. Just map every basis vector to its image and write the results in a matrix. 
...
Multiplying a vector with that matrix (remember: multiply the row in the matrix with the column of the vector and sum it up) we get its reflection.
\end{Ex}
